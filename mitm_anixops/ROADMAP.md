...
The latest published stable artifact is `v1.4.5`. The `v1.1.x` train is
...
change-summary evidence introduced for future releases. The `v1.3.6` follow-up
records the `v1.3.5` public release and turns the release evidence index into
a versioned freshness gate: stable patch releases must prove `latestStable`
...
required latest stable evidence before publication. The `v1.4.1` follow-up
records the `v1.4.0` public release in the evidence index and makes `v1.4.1`
the next stable patch target for the same freshness gate. The `v1.4.2`
follow-up records the `v1.4.1` public release and advances that target to
`v1.4.2`. The `v1.4.3` follow-up records the `v1.4.2` public release and
requires the machine-readable evidence entries to remain sorted
newest-to-oldest. The `v1.4.4` follow-up records the `v1.4.3` public release
and adds negative fixture coverage proving unsorted evidence entries fail the
release evidence index check. The `v1.4.5` follow-up records the `v1.4.4`
public release and requires retained evidence entries to remain contiguous
across adjacent patch releases. The `v1.4.6` follow-up records the `v1.4.5`
public release and requires every retained evidence entry to carry publication

...
- Added a `v1.4.5` release evidence index rollover that records `v1.4.4` as
  ...
  adjacent patch releases.
- Added a `v1.4.4` release evidence index rollover that records `v1.4.3` as
  the latest public stable Release, makes `v1.4.4` the next stable patch
  target, and adds a CI-covered negative fixture proving unsorted evidence
  entries fail the index check.
- Added a `v1.4.3` release evidence index rollover that records `v1.4.2` as
  the latest public stable Release, makes `v1.4.3` the next stable patch
  target, and requires index entries to stay sorted newest-to-oldest.
- Added a `v1.4.2` release evidence index rollover that records `v1.4.1` as
  the latest public stable Release and makes `v1.4.2` the next stable patch
  target for the versioned freshness gate.
- Added a `v1.4.1` release evidence index rollover that records `v1.4.0` as
  the latest public stable Release and makes `v1.4.1` the next stable patch
  target for the versioned freshness gate.
...
- Added a `v1.3.6` release evidence index freshness gate that records `v1.3.5`
  as the latest public stable Release and requires stable patch releases to
  prove `latestStable` matches the previous stable patch for the target
...
- Added a `v1.2.4` release workflow run evidence field set so
  tag-triggered releases and release dry-runs record the producing workflow run
  ID and URL in generated manifests, release notes, GitHub Step Summaries, and
  metadata gates.

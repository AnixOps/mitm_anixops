...
sh "$ROOT/scripts/release-evidence-index-check.sh" v1.3.6 > "$pass_output"
grep -q "release evidence index check passed" "$pass_output"

if sh "$ROOT/scripts/release-evidence-index-check.sh" v1.3.7 > "$fail_output" 2>&1; then
	printf '%s\n' "release evidence index check should fail when latestStable is not the previous patch" >&2
	exit 1
fi
grep -q "latestStable must be v1.3.6 for release v1.3.7" "$fail_output"

sh "$ROOT/scripts/release-evidence-index-check.sh" v1.3.6-rc.1 > "$prerelease_output"

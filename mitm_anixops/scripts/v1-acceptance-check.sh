...
sh "$ROOT/scripts/release-evidence-index-check-test.sh" >/dev/null
...
grep -q "release-evidence-index-freshness-policy=stable-patch-or-declared-boundary-release-requires-latestStable-evidence" "$RELEASE_GATE"

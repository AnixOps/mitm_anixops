...
- Full NSRegularExpression/PCRE compatibility beyond POSIX ERE plus the tested leading `(?i)`/`(?s)` prefixes,
  shorthand classes, control, hex, and Unicode escapes, lazy quantifier normalization, absolute anchors, non-capturing
  groups, named capture groups, and quoted literal matching subset.

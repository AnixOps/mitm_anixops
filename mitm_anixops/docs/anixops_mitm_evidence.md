...
- URL rewrite uses POSIX ERE with a tested leading `(?i)` case-insensitive prefix, PCRE shorthand classes, absolute
  anchors, non-capturing groups, named capture groups, and quoted literal matching subset. This is portable C, but not
  full NSRegularExpression/PCRE compatibility.

...
| Stash URL rewrite reject | YAML `http.url-rewrite` `pattern - reject` | Supported parser/reject-intent subset | `Stash.UrlRewrite.yaml`, `Stash.UrlRewrite.Malformed.yaml`, `config/stash_url_rewrite_fixture_maps_reject_subset`, `config/stash_url_rewrite_malformed_fixture_rejects_invalid_regex`; redirect and direct/proxy route selection remain migration-guarded |
...
| Shadowrocket rule reject | `[Rule]` `URL-REGEX,<pattern>,REJECT` | Supported parser/reject-intent subset | `Shadowrocket.RuleReject.conf`, `Shadowrocket.RuleReject.Malformed.conf`, `config/shadowrocket_rule_reject_fixture_maps_url_regex_rejects`, `config/shadowrocket_rule_reject_malformed_fixture_rejects_invalid_regex`; direct/proxy route selection remains migration-guarded |

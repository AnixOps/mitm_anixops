...
| Stash HTTP MITM hosts | YAML `http.mitm` host list | Supported parser/MITM-host subset | `Stash.HttpMitm.yaml`, `Stash.HttpMitm.Malformed.yaml`, `Stash.HttpMitm.PortSpecificUnsupported.yaml`, `config/stash_http_mitm_fixture_exposes_host_patterns`, `config/stash_http_mitm_malformed_fixture_rejects_invalid_host`, `config/stash_http_mitm_port_specific_fixture_stays_unsupported`; `host:*` entries normalize to host-only policy patterns; Stash app-level routing/proxy syntax remains migration-guarded |

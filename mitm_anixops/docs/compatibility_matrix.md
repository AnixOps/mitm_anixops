...
| Loon rule reject | `[Rule]` `DOMAIN,<host>,REJECT`, `DOMAIN-KEYWORD,<keyword>,REJECT`, `DOMAIN-SUFFIX,<host>,REJECT` | Supported parser/reject-intent subset | `Loon.RuleDomainReject.plugin`, `Loon.RuleDomainReject.Malformed.plugin`, `Loon.RuleDomainExactReject.plugin`, `Loon.RuleDomainExactReject.Malformed.plugin`, `Loon.RuleDomainKeywordReject.plugin`, `Loon.RuleDomainKeywordReject.Malformed.plugin`, `config/loon_rule_domain_reject_fixture_maps_domain_suffix_rejects`, `config/loon_rule_domain_exact_reject_fixture_maps_exact_domain_rejects`, `config/loon_rule_domain_keyword_reject_fixture_maps_domain_keyword_rejects`, `config/loon_rule_domain_reject_malformed_fixture_rejects_invalid_domain`, `config/loon_rule_domain_exact_reject_malformed_fixture_rejects_invalid_domain`, `config/loon_rule_domain_keyword_reject_malformed_fixture_rejects_invalid_keyword`; direct/proxy route selection remains adapter-owned |

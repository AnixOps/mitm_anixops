...
| Shadowrocket rule reject | `[Rule]` `URL-REGEX,<pattern>,REJECT`, `DOMAIN-SUFFIX,<host>,REJECT` | Supported parser/reject-intent subset | `Shadowrocket.RuleReject.conf`, `Shadowrocket.RuleReject.Malformed.conf`, `Shadowrocket.RuleDomainReject.conf`, `Shadowrocket.RuleDomainReject.Malformed.conf`, `config/shadowrocket_rule_reject_fixture_maps_url_regex_rejects`, `config/shadowrocket_rule_domain_reject_fixture_maps_domain_suffix_rejects`, `config/shadowrocket_rule_reject_malformed_fixture_rejects_invalid_regex`, `config/shadowrocket_rule_domain_reject_malformed_fixture_rejects_invalid_domain`; direct/proxy route selection remains migration-guarded |

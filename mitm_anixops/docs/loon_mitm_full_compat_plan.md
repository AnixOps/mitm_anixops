...
- 加 `LOON_STRICT`、`SURGE_STRICT`、`QX_STRICT`、`PORTABLE` mode。
...
| P1 | Header semantics | case-insensitive lookup、multi-value append/replace/delete、Set-Cookie 特例策略 | header matrix fixture 覆盖 request 和 response |
| P2 | Malformed behavior | LOON/SURGE/QX strict mode 与 portable mode 的 ignored/rejected 差异 | 同一 malformed corpus 在不同 profile 下输出不同但稳定诊断 |
...
| P1 | Script cache | remote script fetch/cache/digest/offline bundle | digest mismatch、cache miss、offline mode 均有 fixture |
...
| P1 | PCRE2 | 可选 PCRE2 backend，支持 lookaround、backreference、Unicode property、word boundary | PCRE2 feature matrix 通过 |
| P1 | Replacement | `$1`、`${1}`、`\1`、named capture replacement 行为统一 | URL redirect、body regex、header regex 三类 golden test |
| P1 | Empty match | global replace 对空匹配不死循环，行为固定 | `^`、`$`、`\b`、`.*?` fixture 通过 |

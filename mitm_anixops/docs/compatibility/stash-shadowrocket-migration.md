...
- Shadowrocket has a dedicated rule-reject source contract for a narrow
  `[Rule]` `URL-REGEX`, `DOMAIN`, and `DOMAIN-SUFFIX` reject subset.
...
- Shadowrocket app-level profile syntax outside common config and `[Rule]`
  URL-regex/domain/domain-suffix reject remains a migration guard unless a
...
- Shadowrocket `[Rule]` `URL-REGEX`, `DOMAIN`, and `DOMAIN-SUFFIX` reject
  syntax has partial parser support as policy-core reject intent.
...
- `[Rule]` `URL-REGEX`, `DOMAIN`, and `DOMAIN-SUFFIX` reject entries register
  request-phase rewrite reject decisions;

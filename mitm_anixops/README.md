...
- POSIX ERE regex matching with a supported leading `(?i)` case-insensitive prefix, PCRE shorthand classes
  `\d`, `\w`, and `\s`, PCRE hex byte escapes such as `\x2e`, PCRE Unicode escapes such as `\u00e9`, lazy quantifier
  suffixes normalized to greedy equivalents, plus PCRE absolute anchors `\A`, `\z`, and `\Z`.
